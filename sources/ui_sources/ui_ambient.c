...
static	int		light_adding(t_rtui_min *win_amb, t_rt *rt, t_sdl *sdl)
{
	t_light	*new;
	...
	new->intensity = get_double(win_amb->en_intens.text, '.');
	if (new->intensity > 1)
		new->intensity = 1;
	new->type = AMBIENT;
	new->next = NULL;
...
static	void	button_event_qlt(t_rtui_min *win_amb, t_rt *rt, t_sdl *sdl)
{
	...
	{
		if (light_adding(win_amb, rt, sdl))
